...
(function(__context){
  ...
  function sanitizeShareName(value){
    const fallback = i18n("filelink_share_default") || "Freigabe";
  ...
  async function buildHtmlBlock(result, request){
    ...
    const introLine = i18n("filelink_html_intro_line");
    if (introLine){
    ...
    const rows = [];
    rows.push(buildTableRow(i18n("filelink_html_download_label"), downloadLink));
    if (result.password){
      ...
      rows.push(buildTableRow(i18n("filelink_html_password_label"), badge));
    }
    if (result.expireDate){
      rows.push(buildTableRow(i18n("filelink_html_expire_label"), escapeHtml(result.expireDate)));
    }
    rows.push(buildTableRow(i18n("filelink_html_permissions_label"), buildPermissionsBadges(result.permissions)));
    const nextcloudAnchor = `<a href="https://nextcloud.com/" style="color:#0067c0;text-decoration:none;">Nextcloud</a>`;
    const footer = i18n("filelink_html_footer", [nextcloudAnchor]) || "";
    return `
<div style="font-family:Calibri,'Segoe UI',Arial,sans-serif;font-size:11pt;color:#1f1f1f;margin:16px 0;">
  ...
  function buildPermissionsBadges(perms){
    const safePerms = perms || {};
    const entries = [
      { label: i18n("filelink_permission_read"), enabled: !!safePerms.read },
      { label: i18n("filelink_permission_create"), enabled: !!safePerms.create },
      { label: i18n("filelink_permission_write"), enabled: !!safePerms.write },
      { label: i18n("filelink_permission_delete"), enabled: !!safePerms.delete }
    ];
  ...
  async function getFileLinkBasePath(){
    ...
    const stored = await browser.storage.local.get(["fileLinkBasePath"]);
    return stored.fileLinkBasePath || DEFAULT_BASE_PATH;
  }
  ...
  async function updateShareDetails({ shareInfo, noteEnabled, note } = {}){
    if (!shareInfo?.shareId){
      throw new Error(i18n("filelink_error_upload_required"));
    }
    const opts = await NCCore.getOpts();
    if (!opts.baseUrl || !opts.user || !opts.appPass){
      throw new Error(i18n("error_credentials_missing"));
